...
# cluster energies, and energy-weighted centroids.
...
function push!(s::PhotonStack; nm=0, parent_region=:CB, region=:TPC,
...
@testset "3. two :TPC rows Δz < 3mm → 1 cluster, ec summed" begin
    ...
    push!(s; region=:TPC, interaction=INT_COMPTON,
...
@testset "5. mixed regions: only :TPC clustered" begin
    s = PhotonStack()
    push!(s; region=:Skin,  x=0.0, y=0.0, z=10.0, edep=0.4)
...
@testset "6. :BELOW_THRESH respects region filter" begin
    s = PhotonStack()
    # In :TPC: contributes
    push!(s; region=:TPC, interaction=INT_BELOW_THRESH,
